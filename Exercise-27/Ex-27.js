"use strict";
// Ex-27: Alien Colors #3: Turn your if-else chain from Exercise 5-4 into an if-else chain.
// • If the alien is green, print a message that the player earned 5 points.
// • If the alien is yellow, print a message that the player earned 10 points.
// • If the alien is red, print a message that the player earned 15 points.
// • Write three versions of this program, making sure each message is printed for the appropriate color alien.
let alien_color4 = 'green';
if (alien_color4 === 'green') {
    console.log("Hurrah! You just earned 5 Points for shooting the green alien.");
}
else if (alien_color4 === 'yellow') {
    console.log("Hurrah! You just earned 10 Points for shooting the yellow alien.");
}
else if (alien_color4 === 'red') {
    console.log("Hurrah! You just earned 15 Points for shooting the red alien.");
}
let alien_color5 = 'yellow';
if (alien_color5 === 'green') {
    console.log("Hurrah! You just earned 5 Points for shooting the green alien.");
}
else if (alien_color5 === 'yellow') {
    console.log("Hurrah! You just earned 10 Points for shooting the yellow alien.");
}
else if (alien_color5 === 'red') {
    console.log("Hurrah! You just earned 15 Points for shooting the red alien.");
}
let alien_color6 = 'red';
if (alien_color6 === 'green') {
    console.log("Hurrah! You just earned 5 Points for shooting the green alien.");
}
else if (alien_color6 === 'yellow') {
    console.log("Hurrah! You just earned 10 Points for shooting the yellow alien.");
}
else if (alien_color6 === 'red') {
    console.log("Hurrah! You just earned 15 Points for shooting the red alien.");
}
